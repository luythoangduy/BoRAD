...
class MAMBAADLGC(nn.Module):
    def __init__(self, model_t, model_s, dp=False):
        super(MAMBAADLGC, self).__init__()
        self.net_t = get_model(model_t)
        self.mff_oce = MFF_OCE(Bottleneck, 3)
        self.net_s = MambaUPNet(depths_decoder=model_s['depths_decoder'], scan_type=model_s['scan_type'],
                                num_direction=model_s['num_direction'])
        ...
        self.frozen_layers = ['net_t']
        self.proj_layer = MultiProjectionLayer(base=16, dp=dp)
...
@MODEL.register_module
def mambaad_lgc(pretrained=False, **kwargs):
    model = MAMBAADLGC(**kwargs)
    return model
...
if __name__ == '__main__':
    ...
    model_t.kwargs = dict(pretrained=True, checkpoint_path='',
    ...
    net = MAMBAADLGC(model_t, model_s).cuda()
    net.eval()
    y = net(x)
    ...
    with torch.no_grad():
        ...
        for _ in range(cnt):
            y = net(x)
        t_e = get_timepc()
        print(f'FLOPs: {flops:.2f}G, Params: {params:.2f}M, Time: {t_e - t_s:.2f}s')
