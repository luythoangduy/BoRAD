    ...
    def __init__(self, cfg, train=True, transform=None, target_transform=None):
        ...
        for root in self.root:
            ...
            if name in ['mvtec', 'coco', 'visa', 'medical', 'btad', 'mpdd', 'mad_sim', 'mad_real',
                ...
                meta_info = meta_info['train' if self.train else 'test']
                cls_names = list(meta_info.keys())
                self.cls_names.extend(cls_names)
                for cls_name in cls_names:
                ...
                for cls_name in cls_names:
                    if cls_name.split('.')[0] not in real_cls_names:
                        real_cls_names.append(cls_name.split('.')[0])
                real_cls_names.sort()
                self.cls_names.extend(real_cls_names)
                meta_info = dict()
                for cls_name in real_cls_names:
                    data_cls_all = []
                    cls_info = json.load(open(f'{root}/{cls_name}.json', 'r'))
                    data_cls = cls_info['train' if self.train else 'test']
                    for data in data_cls:
                        if data['anomaly_class'] == 'OK':
                            info_img = dict(
                                img_path=f"{root}/{cls_name}/{data['image_path']}",
                                mask_path='',
                                cls_name=cls_name,
                                specie_name='',
                                anomaly=0,
                            ...
                            info_img = dict(
                                img_path=f"{root}/{cls_name}/{data['image_path']}",
                                mask_path=f"{root}/{cls_name}/{data['mask_path']}",
                                cls_name=cls_name,
                                specie_name=data['anomaly_class'],
                                anomaly=1,
                            )
...
@DATA.register_module
class UnifiedDraem(data.Dataset):
    def __init__(self, cfg, train=True, transform=None, target_transform=None):
        self.root = cfg.data.root
        if not isinstance(self.root, list):
            self.root = [self.root]
        ...
        for root in self.root:
            name = root.split('/')[-1]
            if name in ['mvtec', 'coco', 'visa', 'medical', 'btad', 'mpdd', 'mad_sim', 'mad_real', 'MVTec_AD',
    ...
    def augment_image(self, image, anomaly_source_path):
        ...
        perlin_noise = generate_perlin_noise_2d((self.resize_shape[0], self.resize_shape[1]),
...
@DATA.register_module
class UnifiedDeSTSeg(data.Dataset):
    def __init__(self, cfg, train=True, transform=None, target_transform=None):
        self.root = cfg.data.root
        self.train = train
        self.transform = transform
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            view_img = self.aug_transform(image)

            augmented_image = self.transform(augmented_image) if self.transform is not None else augmented_image
            sample = {'img': img, "img_mask": anomaly_mask, 'cls_name': cls_name, 'label': self.cls2idx[cls_name],
                      'augmented_image': augmented_image, 'anomaly': has_anomaly, 'img_path': img_path,
                      'aug_img': view_img}
            return sample
        else:
